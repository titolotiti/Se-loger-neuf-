...
function looksLikeProgramArray(arr: unknown[]): boolean {
  return arr.slice(0, 3).some((item) => {
    if (!item || typeof item !== "object") return false;
    const obj = item as Record<string, unknown>;
    const hasUrl = ["url", "permalink", "detailUrl"].some(
      (k) => typeof obj[k] === "string" && (obj[k] as string).includes("/")
    );
    const hasName = ["title", "name", "titre", "nom"].some((k) => typeof obj[k] === "string" && obj[k]);
    const hasPrice = ["prixMin", "prix", "price", "budgetMin"].some((k) => obj[k] != null);
    return (hasUrl || hasName) && hasPrice;
  });
}
...
function buildProgramFromSearchItem(
  ...
  extractedAt: string,
  seen: Set<string>
): NeufProgram | "category" | "duplicate" | null {
  const rawUrl = getStr(item, "url", "permalink", "detailUrl", "href", "link");
  ...
  if (!isProgramDetailUrl(fullUrl)) return "category";
  if (seen.has(fullUrl)) return "duplicate";
  seen.add(fullUrl);

  const name = getStr(item, "title", "name", "titre", "libelle", "nom", "programName", "programmeNom", "programmeTitle");
  if (!name || !isValidProgramName(name)) {
    console.warn(`[parser:search] Nom invalide "${name}" → ${fullUrl}`);
    return null;
  }

  const developer = getStr(item, "promoteur", "developer", "agency", "advertiser", "promoterName", "commercialisateur", "agence", "developerName", "promoterLabel");
  const itemCity = getStr(item, "city", "ville", "commune", "localite") ?? city;
  const itemPostalCode = getStr(item, "postalCode", "codePostal", "cp", "zipCode") ?? postalCode;
  const deliveryDate = getStr(item, "deliveryDate", "livraison", "datelivraison", "delivery", "dateLivraison", "livraisonDate", "deliveryQuarter");

  const urlPathId = /\/(\d+)\/?$/.exec(new URL(fullUrl).pathname)?.[1];
  const programId = getIdStr(item, "id", "idAnnonce", "classifiedId", "programmeId", "programId") ?? urlPathId ?? `prog_${++programCounter}_${Date.now()}`;
...
export function parseSearchResultsPrograms(
  ...
  if (!raw) {
    ...
    return { programs: [], rawItemCount: 0, categorySkipped: 0, duplicatesSkipped: 0 };
  }
  ...
  for (const rawItem of items) {
    ...
    if (result === "category") {
      ...
      console.log(`[parser:search] ✓ "${result.programName}" — ${result.listings.length} lot(s) — ${result.url}`);
    }
  }

  console.log(`[parser:search] → ${programs.length} programme(s) (${categorySkipped} catégories, ${duplicatesSkipped} doublons ignorés)`);
  return { programs, rawItemCount: items.length, categorySkipped, duplicatesSkipped };
}
