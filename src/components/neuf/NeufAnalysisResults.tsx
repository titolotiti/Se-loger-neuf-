...
import type { NeufAnalysisResult, NeufProgram, NeufTypology, ScrapeReport } from "@/types/neuf";
...
function ScrapeReportPanel({ report }: { report: ScrapeReport }) {
  const [expanded, setExpanded] = useState(false);

  const hasBlockage =
    report.blocked403 > 0 || report.cloudflareBlocks > 0 || report.jsOnlyPages > 0 || report.timeouts > 0;
  ...
  return (
    <div className={`rounded-lg border p-4 ${hasBlockage ? "bg-red-50 border-red-300" : "bg-gray-50 border-gray-200"}`}>
      {/* Diagnostic principal */}
      {report.diagnosis && (
        <div className="mb-3 flex gap-2 items-start">
          <span className="text-xl shrink-0">⛔</span>
          <div>
            <p className="font-semibold text-red-800 text-sm">Accès SeLoger Neuf bloqué</p>
            <p className="text-red-700 text-sm mt-0.5">{report.diagnosis}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-3 text-xs mb-3">
        ...
        <Stat label="Liens trouvés" value={report.totalLinksFound} />
...
export default function NeufAnalysisResults({ result, onExport, exportLoading }: Props) {
  ...
  const isBlocked = result.scrapeReport?.diagnosis != null && result.programs.length === 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Rapport de scraping — affiché si problème ou si 0 résultat */}
      {result.scrapeReport && (isBlocked || result.programs.length === 0) && (
        <ScrapeReportPanel report={result.scrapeReport} />
      )}

      {/* Warnings (autres) */}
      {result.warnings.filter((w) => !w.startsWith("⛔")).length > 0 && (
