...
import type { LinkType } from "@/types/neuf";
...
export function classifyLink(link: ProgramLink): LinkType {
  ...
  if (path.includes("/bien-programme/")) {
    const match = /\/bien-programme\/([^/?]+)\/?$/.exec(path);
    if (!match) return "category";
    const slug = match[1];
    if (/^\d+-pieces?$/.test(slug)) return "category";
    if (BIEN_PROGRAMME_FILTER_SLUGS.has(slug)) return "category";
    return "program";
  }
