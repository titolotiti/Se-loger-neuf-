...
// Types locaux pour l'export du collecteur : évite de dépendre d'un changement de src/types/neuf.ts
type CollectedLot = {
...
function buildAdresseSheet(wb: ExcelJS.Workbook, result: NeufAnalysisResult) {
  ...
  for (const prog of result.programs) {
    ...
    // Bug fix: do not write a bare number like "4" as the address
    const addrStr = typeof prog.address === "string" && /[a-zA-ZÀ-ÿ]/.test(prog.address)
      ? prog.address
      : null;
    row.getCell(3).value = addrStr ?? "Non communiqué";
...
function buildCollectedAdresseRowIndex(programs: CollectedProgram[]): Map<string, number> {
  const map = new Map<string, number>();
  programs.forEach((prog, i) => {
    map.set(prog.id, 4 + i);
  });
...
// ─── COLLECTED : ONGLET ADRESSE ───────────────────────────────────────────────

function buildCollectedAdresseSheet(wb: ExcelJS.Workbook, payload: ExportCollectedPayload) {
  const ws = wb.addWorksheet("Adresse");

  ws.mergeCells("A1:G1");
  const titleCell = ws.getCell("A1");
  titleCell.value = payload.label;
  titleCell.fill = darkFill(COLOR_HEADER_DARK);
  titleCell.font = { bold: true, size: 14, color: { argb: `FF${COLOR_WHITE}` } };
  titleCell.alignment = { vertical: "middle", horizontal: "center" };
  ws.getRow(1).height = 30;
  ...
  for (const prog of payload.programs) {
    const row = ws.getRow(rowIdx);
    row.getCell(1).value = prog.id;
    row.getCell(2).value = prog.programName;
    const addrStr =
      typeof prog.address === "string" && /[a-zA-ZÀ-ÿ]/.test(prog.address)
        ? prog.address
        : null;
    row.getCell(3).value = addrStr ?? "";
    row.getCell(4).value = prog.city || "Commune inconnue";
    row.getCell(5).value = prog.postalCode ?? "";
    row.getCell(6).value = prog.promoter ?? "";
    const urlCell = row.getCell(7);
    if (prog.url) {
      urlCell.value = { text: prog.url, hyperlink: prog.url };
      urlCell.font = { color: { argb: "FF0563C1" }, underline: true, size: 10 };
...
// ─── COLLECTED : ONGLET DATA ──────────────────────────────────────────────────

function buildCollectedDataSheet(wb: ExcelJS.Workbook, payload: ExportCollectedPayload) {
  const ws = wb.addWorksheet("Data");
  const ncols = DATA_COLS.length;

  ws.mergeCells(`A1:${colLetter(ncols)}1`);
  const t = ws.getCell("A1");
  t.value = `Données lots — ${payload.label}`;
  t.fill = darkFill(COLOR_HEADER_DARK);
  t.font = { bold: true, size: 13, color: { argb: `FF${COLOR_WHITE}` } };
  t.alignment = { vertical: "middle", horizontal: "center" };
  ws.getRow(1).height = 28;
  ...
  w.value =
    "⚠️ Prix de commercialisation — SeLoger Neuf. " +
    "Lots sans surface/prix exclus des statistiques.";
  w.fill = darkFill("FFF2CC");
  w.font = { italic: true, size: 10, color: { argb: "FFCC7700" } };
  w.alignment = { vertical: "middle", horizontal: "center", wrapText: true };
  ws.getRow(2).height = 28;
  ...
  for (const prog of payload.programs) {
    if (prog.lots.length === 0) continue; // programmes sans lots : pas de ligne Data

    for (const lot of prog.lots) {
      const hasSurface = lot.surfaceM2 != null && lot.surfaceM2 > 0;
      const hasPrice = lot.priceEur != null && lot.priceEur > 0;
      const exclu = !hasSurface || !hasPrice;
      const raison = !hasSurface
      ...
      row.getCell(1).value = prog.id;
      row.getCell(2).value = prog.programName;
      row.getCell(3).value = prog.promoter ?? "";
      row.getCell(4).value = prog.city || "Commune inconnue";
      ...
      row.getCell(6).value = prog.totalUnits ?? null;
      row.getCell(7).value = lot.availableCount;
      row.getCell(8).value = prog.deliveryDate ?? "Non communiqué";
      row.getCell(9).value = ""; // parking non extrait par le bookmarklet

      row.getCell(10).value = lot.typology ?? lot.rawTypology ?? "";
      row.getCell(11).value = lot.surfaceM2;
      row.getCell(12).value = lot.priceEur;
      ...
      row.getCell(14).value = exclu ? "Oui" : "Non";
      row.getCell(15).value = raison;
...
// ─── COLLECTED : ONGLET SYNTHÈSE ─────────────────────────────────────────────

function buildCollectedSyntheseSheet(wb: ExcelJS.Workbook, payload: ExportCollectedPayload) {
  const ws = wb.addWorksheet("Offre neuve — collecte");
  const ncols = SYNTH_COLS.length;

  // grouper par ville
  const byCity = new Map<string, CollectedProgram[]>();
  for (const prog of payload.programs) {
    const key = prog.city || "Commune non renseignée";
    if (!byCity.has(key)) byCity.set(key, []);
    byCity.get(key)!.push(prog);
  }

  const adresseRows = buildCollectedAdresseRowIndex(payload.programs);

  // ── Titre ──
  ws.mergeCells(`A1:${colLetter(ncols)}1`);
  const t1 = ws.getCell("A1");
  t1.value = payload.label;
  t1.fill = darkFill(COLOR_HEADER_DARK);
  t1.font = { bold: true, size: 13, color: { argb: `FF${COLOR_WHITE}` } };
  t1.alignment = { vertical: "middle", horizontal: "center" };
  ws.getRow(1).height = 32;
  ...
  for (const cityName of orderedCities) {
    ...
    ws.mergeCells(`A${rowIdx}:${colLetter(ncols)}${rowIdx}`);
    const sepCell = ws.getCell(`A${rowIdx}`);
    sepCell.value = `▶ ${cityName}`;
    sepCell.fill = darkFill(COLOR_HEADER_DARK);
    sepCell.font = { bold: true, size: 11, color: { argb: `FF${COLOR_WHITE}` } };
    sepCell.alignment = { vertical: "middle", horizontal: "left" };
    ws.getRow(rowIdx).height = 22;
    rowIdx++;
    ...
    for (const prog of cityProgs) {
      const adresseRow = adresseRows.get(prog.id);
      if (!adresseRow) continue;

      const r = rowIdx;
      const row = ws.getRow(r);
      const nameRef = `$A${r}`;
      const noLots = prog.lots.length === 0;

      // A: Nom → Adresse col B
      row.getCell(1).value = { formula: `Adresse!B${adresseRow}` };
      // B: Promoteur → Adresse col F
      row.getCell(2).value = { formula: `Adresse!F${adresseRow}` };
      // C: Commune → Adresse col D
      row.getCell(3).value = { formula: `Adresse!D${adresseRow}` };
      // D: URL → Adresse col G
      row.getCell(4).value = { formula: `Adresse!G${adresseRow}` };
      // E: Livraison → Data col H (première correspondance)
      row.getCell(5).value = {
        formula: `IFERROR(INDEX(Data!$H:$H,MATCH($A${r},Data!$B:$B,0)),"")`,
      };

      if (noLots) {
        // Programmes sans lots : cellules vides + note
        for (let c = 6; c <= ncols; c++) {
          row.getCell(c).value = "";
        }
        // Indiquer "lots non détectés" visuellement
        row.getCell(6).value = "Lots non détectés";
        for (let c = 1; c <= ncols; c++) {
          const cell = row.getCell(c);
          cell.fill = darkFill("FFF2CC");
          cell.font = { italic: true, size: 10, color: { argb: "FF7D4F00" } };
          cell.border = thin();
          cell.alignment = { vertical: "middle", wrapText: true };
        }
      } else {
        // F–O: Surface et Prix/m² par typologie
        TYPOS.forEach((typo, ti) => {
          const surfCol = 6 + ti * 2;
          const priceCol = 7 + ti * 2;
          row.getCell(surfCol).value = avgFormula("K", "Data!$B:$B", nameRef, typo);
...
export async function exportCollectedToExcel(payload: ExportCollectedPayload): Promise<Buffer> {
  ...
  buildCollectedAdresseSheet(wb, payload);
  buildCollectedDataSheet(wb, payload);
  buildCollectedSyntheseSheet(wb, payload);
